...
function NavItem({ to, icon, label, visivel = true, externo = false, onClick }) {
      ...
      <div style={{padding:'.85rem 1.25rem',borderBottom:'0.5px solid #E0DDD5',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        ...
        {isMobile && (
          <button onClick={() => setMenuAberto(false)}
            style={{border:'none',background:'none',fontSize:20,cursor:'pointer',color:'#888780',padding:'4px'}}>✕</button>
        )}
      </div>
        ...
        {isMobile && (
          <div style={{background:'#fff',borderBottom:'0.5px solid #E0DDD5',padding:'.6rem 1rem',display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
            <button onClick={() => setMenuAberto(true)}
              style={{border:'none',background:'none',fontSize:22,cursor:'pointer',color:'#5F5E5A',padding:'2px 4px',lineHeight:1}}>
              ☰
            </button>
            <img src="/logo.png" alt="CAPETTE" style={{height:30,width:'auto',objectFit:'contain'}}
