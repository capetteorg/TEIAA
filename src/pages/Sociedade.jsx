...
export default function Sociedade() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F8F7F2' }}>
      ...
      <div style={{ background:'#fff', borderBottom:'0.5px solid #E0DDD5', padding:'1.25rem 1.5rem' }}>
        <div style={{ maxWidth:1000, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
          <div style={{ display:'flex', alignItems:'center', gap:14 }}>
            <img src="/logo.png" alt="CAPETTE" style={{ height:48, width:'auto', objectFit:'contain' }}
              onError={e=>{ e.target.style.display='none'; e.target.nextSibling.style.display='flex' }} />
            <div style={{ display:'none', gap:2, alignItems:'center' }}>
              {LOGO.map(([l,c]) => <span key={l+c} style={{ fontSize:20, fontWeight:700, color:c }}>{l}</span>)}
            </div>
            <div>
              <div style={{ fontSize:15, fontWeight:600, color:'#2C2C2A' }}>Portal de Transparência</div>
              <div style={{ fontSize:11, color:'#888780' }}>
                {instituicao?.nome_completo || 'Casa do Pequeno Trabalhador de Teresópolis'} · CNPJ {instituicao?.cnpj || '29.213.717/0001-01'}
              </div>
            </div>
          </div>
          <a href="/login" style={{ fontSize:12, color:AZUL, textDecoration:'none', padding:'6px 14px', border:`0.5px solid ${AZUL}`, borderRadius:8 }}>← Área interna</a>
        </div>
      </div>
      ...
      {/* Rodapé */}
      <div style={{ background:'#2C2C2A', padding:'1.25rem 1.5rem', marginTop:'2rem' }}>
        <div style={{ maxWidth:1000, margin:'0 auto', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:8 }}>
          ...
          <div style={{ fontSize:11, color:'#888780' }}>Portal de Transparência Pública</div>
        </div>
      </div>
    </div>
  )
